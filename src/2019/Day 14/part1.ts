...
const getBasicElements = (element: IElement, units: number): IElement[] => {
  ...
  return element.producedBy
    .map((child) => {
      const eroot = elementsProduction.find((ep) => ep.element.name == child.name).element;
...
const totalOre = names.reduce((a, name) => {
  ...
  const neededUnits = basicElements.filter((e) => e.name == name).reduce((ac, el) => ac + el.units, 0);

  const packages = Math.ceil(neededUnits / element.units);
  const unitProduced = packages * element.units;
  console.log(
    `${neededUnits} of ${name} in ${packages} pck of ${element.units}  (${unitProduced} in all packages), ${
      (unitProduced / element.units) * element.producedBy[0].units
    } ORE`
  );
